...
public class GSTBillController {
    ...
    public GSTBillController(@Lazy GSTBillService gstBillService) {
        this.gstBillService = gstBillService;
    }
    ...
    @GetMapping("/filter")
    @Operation(summary = "Filter by dates", description = "API for filter GST bills by date ranges", responses = {
            @ApiResponse(responseCode = "400", content = @Content(schema = @Schema(implementation = LinkedHashMap.class))),
            @ApiResponse(responseCode = "401", content = @Content(schema = @Schema(example = "Unauthorized. Please login and provide token in header"))),
            @ApiResponse(responseCode = "404", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))})
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<List<GSTBill>> filterByDates(
            @RequestParam @DateTimeFormat(pattern = "dd-MM-yyyy", iso = DateTimeFormat.ISO.DATE) Date fromDate,
            @RequestParam @DateTimeFormat(pattern = "dd-MM-yyyy", iso = DateTimeFormat.ISO.DATE) Date toDate) {
        return ResponseEntity.ok(gstBillService.filterBillsByDates(fromDate, toDate));
    }

    @GetMapping("/report")
    @Operation(summary = "Generate report", description = "API for generate GST bills report", responses = {
            @ApiResponse(responseCode = "401", content = @Content(schema = @Schema(example = "Unauthorized. Please login and provide token in header"))),
            @ApiResponse(responseCode = "404", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))})
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<GSTReportResponse> generateReport(
            @RequestParam(required = false) Integer months,
            @RequestParam(required = false) @DateTimeFormat(pattern = "dd-MM-yyyy", iso = DateTimeFormat.ISO.DATE) Date fromDate,
            @RequestParam(required = false) @DateTimeFormat(pattern = "dd-MM-yyyy", iso = DateTimeFormat.ISO.DATE) Date toDate) {
        return ResponseEntity.ok(gstBillService.generateReports(months, fromDate, toDate));
    }
}
